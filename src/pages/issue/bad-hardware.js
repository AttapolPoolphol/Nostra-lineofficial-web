...
const problems = ['เครื่องรูดบัตรดังไม่หยุด', 'อื่นๆ']

const IssueBadHardwarePage = props => {
  const [businessUnitId, setBusinessUnitId] = useState(null)

  useEffect(() => {
    const liff = window.liff
    liff.init({ liffId: process.env.LIFF_ISSUE_HARDWARE }, lineInit, err =>
  ...
  const lineInit = () => {
    ...
    if (liff.isLoggedIn()) {
      window.liff.getProfile().then(({ userId }) => {
        props.setUid(userId)
        props.setRedirectPath('/issue/bad-hardware')
        props.getCompany()
      })
    } else {
      liff.login()
  ...
  return (
    <Layout title="แจ้งปัญหา" pageName="Hardware">
      <Skeleton active loading={props.companies.length === 0}>
        <Issue
          title="Hardware Product"
          faqUrl="https://nostralogistics.com/product-faq-hardware-product"
          prefixRemark="Hardware problem"
